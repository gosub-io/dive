...
impl Drawable for TabListWidget {
    fn render(&mut self, f: &mut Frame) {
        ...
        for (idx, tab) in self.tab_manager.borrow().tabs.iter().enumerate() {
            items.push(Span::styled(format!("{}) {} - {}", idx, tab.name.clone(), tab.url.clone()), Style::default().fg(Color::White)));
        }

        let block = Block::default().title("Tab List").borders(Borders::ALL).padding(Padding::new(1, 1, 1, 1));

        let list = List::new(items)
            .style(Style::default().fg(Color::White))
            .highlight_style(Style::default().bg(Color::Red).fg(Color::Yellow).add_modifier(Modifier::BOLD))
            .highlight_symbol("> ")
    ...
    fn event_handler(&mut self, queue: &mut CommandQueue, key: KeyEvent) -> anyhow::Result<Option<KeyEvent>> {
        match key.code {
            KeyCode::Esc => {
                queue.push(Command::DestroyWidget { id: "tab_list".into() });
            }
            KeyCode::Down => {
